...
{
    /// <summary>
    ///     clientRecognition 的摘要说明
    /// </summary>
    public class clientRecognition : IHttpHandler
    {
        // 通用引擎参数
                ...
                if (nic.OperationalStatus == OperationalStatus.Up)
                {
                    ...
                    foreach (DataRow row in Global.GetClientMacs().Rows)
                        if (mac.Equals(row.ItemArray[0]))
                        {
                            ...
                            clientId = int.Parse(row.ItemArray[1].ToString());
                            break;
                        }

                    if (macIsInDatabase) break;
                }
                ...
                else
                {
                    ...
                    if (recognitionResult == 0)
                    {
                        ...
                        if (compareRes == 0)
                        {
                            ...
                            var rect = faceInput.rcFace;
                            var cutedImage = Global.CutImage(faceImage, rect.left, rect.top, rect.bottom - rect.top,
                                rect.right - rect.left);
                            var cBase64 = Global.GetBase64FromImage(cutedImage, true);
                            // 保存到usages数据库
                            var sql = "insert into usages values(?cid, ?img, ?uid);";
                            var paras = new MySqlParameter[3];
                            paras[0] = new MySqlParameter("?cid", clientId);
                            paras[1] = new MySqlParameter("?img", cBase64);
                            paras[2] = new MySqlParameter("?uid", returnStruct.userId);
                        ...
                        else if (compareRes == -1)
                        {
                            ...
                            var cBase64 = Global.GetBase64FromImage(cutedImage, true);
                            // 保存到usages数据库
                            var sql = "insert into usages(clientId, image) values(?cid, ?img);";
